...
public:
  ...
  int printparent(int node) { return parent[node]; }
...
int main() {
 // ifstream f("input.txt");
  ...
  vector<int> safeIndexes;
  for (int i = 0; i < (int)actualInput.size(); i++) {
    bool unsafe = false;
    for (int j = 1; j < (int)actualInput[i].size(); j++) {
      vector<int> finder = u[actualInput[i][j]];
      auto it = find(finder.begin(), finder.end(), actualInput[i][j - 1]);
      if (it == finder.end()) {
        unsafe = true;
        break;
      }
  ...
  int sum = 0;
  for (auto k : safeIndexes) {
       sum += actualInput[k][actualInput[k].size()/2];
  }
  cout << sum << endl;
